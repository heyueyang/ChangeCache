...
//static final String findCurrentFileCount = "select count(distinct file_id) from actions" + "where commit_id<? and repository_id=?";
    ...
    public StatisSimulator(int bsize, int psize, int csize, int projid,
        ...
        try {
            ...
            while (allCommits.next()) {
                ...
                while (files.next()) {//遍历该commit_id涉及的所有action
                    ...
                    statisOneFile(cid, cdate, isBugFix, file_id,
                            type, numprefetch);
                }
                numprefetch = 0;
                if (saveToFile == true) {
    ...
    public static void main(String args[]) {
        ...
        //for(int i = 0 ; i < projects.length; i++){
	        ...
	        else
	        {
	            ...
	            sim = new StatisSimulator(blksz, pfsz, csz, pid, crp, start, end, saveToFile);									
    ...
    private static void printSummary(StatisSimulator sim) {
    ...
    private static StatisSimulator tune(int pid)
    {
        StatisSimulator maxsim = null;
        double maxhitrate = 0;
        ...
        for(blksz=onepercent;blksz<UPPER;blksz+=onepercent*2){
            for(pfsz=onepercent;pfsz<UPPER;pfsz+=onepercent*2){
                final StatisSimulator sim = new StatisSimulator(blksz, pfsz,-1, pid, crp, null, null, false);
                sim.initialPreLoad();
                sim.simulate();
                System.out.println(sim.getHitRate());
        ...
        for(CacheReplacement.Policy crtst :CacheReplacement.Policy.values()){
            final StatisSimulator sim = 
                new StatisSimulator(maxsim.blocksize, maxsim.prefetchsize,
    ...
    private static StatisSimulator validate(int pid)
    {
        StatisSimulator maxsim = null;
        double maxhitrate = 0;
